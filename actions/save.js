...
export const save = async (
    ...
    if (existingSavedApplication) {
        ...
        if (
            deletedPendingQualifications.length > 0 &&
            hasPendingResults !== 'No'
        ) {
        ...
        if (signatureData) {
            ...
            const signatureFile = await fetch(signatureData).then((res) =>
                res.blob()
            )
            const uploadedSignature = await utapi.uploadFiles(
                new File(
                    [signatureFile],
                    `${existingSavedApplication.id}-signature.png`
                )
            )
            signatureUrl = uploadedSignature.data.url
    ...
    if (signatureData) {
        ...
        const uploadedSignature = await utapi.uploadFiles(
            new File([signatureFile], `${applicationID}-signature.png`)
        )
        signatureUrl = uploadedSignature.data.url
